...
//! \brief Two-temperature-backed dual-energy formalism for Newtonian ideal-gas MHD.
...
void MHD::SynchronizeDualEnergyFromTotal() {
  if (!use_dual_energy) return;
  ...
  par_for("mhd_2t_dual_energy_sync", DevExeSpace(),
          0, nmb1, gks, gke, gjs, gje, gis, gie,
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    const Real dens = fmax(u0_(m, IDN, k, j, i), eos.dfloor);
