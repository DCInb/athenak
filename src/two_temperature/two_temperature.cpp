...
void TwoTemperature::Initialize(DvceArray5D<Real> &cons, DvceArray5D<Real> &prim,
  ...
  if (!use_material_mixture_) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      thermo(m, effective_charge, k, j, i) = state.effective_charge;
    });
  }
